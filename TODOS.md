...
5. Create the manager contract
    ...
    - should have a poke function which will do the following:
        - 1. First, withdraw all "bought" tokens and "sold" tokens
            - for USDC obtained from selling weth, see the balance delta to figure out how much weth was sold for that amount of USDC. Compute the profit based on the average buy price, then send funds to two places - profit vault and tax vault. Optionally reinvest principle or send principle to profit vault. 
        - 2. if rebalance time has occurred:
            ...
            - next check uniswap price
                - if uniswap price is within lower window, spend `max(PURCH_PCT * dryPowder, MIN_BUY, dryPowder)` of capital on spot to buy WETH
                    - recompute avg buy price
                - else if uniswap price is within upper window, sell `max(DECAY_PCT * WETH_held, MIN_SELL, WETHHeld)`
